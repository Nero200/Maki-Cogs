...
# Original WordClouds cog by FlapJack and aikaterna
# Special thanks to co-author aikaterna for pressing onward
# with this cog when I had lost motivation!
#
# This is WordCloudRe - a refactored version with bug fixes and improvements
...
class WordClouds(commands.Cog):
    ...
    async def wordcloud(self, ctx, *argv):
        ...
        height = 600
        mode = "RGB"
        bg_color = await self.conf.guild(guild).bg_color()
        if bg_color == "clear":
            mode += "A"
            bg_color = None
    ...
    async def _wcset_upload(self, ctx, url: str = None):
        ...
        if attachments:
            ...
            if not filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                await ctx.send("Please upload an image file (PNG, JPG, JPEG, GIF, or BMP).")
                return
            ...
            try:
                ...
                await ctx.send(f"Saving attachment failed: {str(e)}")
                return

        elif url:
            filename = url.split("/")[-1].replace("%20", "_")
            # Basic file validation for URL downloads
            if not filename.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                await ctx.send("URL must point to an image file (PNG, JPG, JPEG, GIF, or BMP).")
                return
            # Sanitize filename to prevent path traversal
            filename = os.path.basename(filename)
            filepath = f"{self.mask_folder}/{filename}"
            try:
                async with self.session.get(url) as new_image:
                    if new_image.status != 200:
                        await ctx.send(f"Failed to download image: HTTP {new_image.status}")
                        return
                    # Check content type
                    content_type = new_image.headers.get('content-type', '')
                    if not content_type.startswith('image/'):
                        await ctx.send("URL does not contain a valid image.")
                        return
    ...
    async def _wcset_bgcolor(self, ctx, color: str):
        ...
        guild = ctx.guild
        await self.conf.guild(guild).bg_color.set(color)
