...
class SimpleRollResult:
    """Mock result object for preset rolls with pre-formatted result string."""
...
class ChimeraDice(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        # In-memory storage for preset test rolls with timestamps.
    ...
    async def _execute_roll(self, ctx: commands.Context, roll_string: str, roll_type: str):
        ...
        try:
            ...
            # Check for preset test rolls first
            user_id = ctx.author.id
            preset_result = None

            if user_id in self.preset_rolls:
                # Clean up expired rolls first
                self._cleanup_expired_preset_rolls()

                # Normalize the dice expression for consistent lookup
                normalized_key = self._normalize_dice_key(roll_string)

                if normalized_key in self.preset_rolls[user_id]:
                    data = self.preset_rolls[user_id][normalized_key]
                    # Handle both new format (dict) and legacy format (list)
                    if isinstance(data, dict) and "values" in data and data["values"]:
                        preset_result = data["values"].pop(0)
                        # Remove entry if no more values
                        if not data["values"]:
                            del self.preset_rolls[user_id][normalized_key]
                            # Clean up empty user dict
                            if not self.preset_rolls[user_id]:
                                del self.preset_rolls[user_id]
                    elif isinstance(data, list) and data:
                        # Legacy format support
                        preset_result = data.pop(0)
                        if not data:
                            del self.preset_rolls[user_id][normalized_key]
                            # Clean up empty user dict
                            if not self.preset_rolls[user_id]:
            ...
            if 'df' in roll_string.lower() or 'f' in roll_string.lower():
                await self._handle_fudge_dice(ctx, roll_string, roll_type, preset_result)
                return
            elif 'dd' in roll_string.lower():
                await self._handle_fallout_dice(ctx, roll_string, roll_type, preset_result)
                return

            # Use d20 library for standard dice with full support for advanced operations
            if preset_result is not None:
                # For preset results with advanced operations, we need special handling
                result = await self._handle_preset_standard_dice(ctx, roll_string, preset_result, roll_type)
            else:
            ...
            # Apply roll type modifications (but not if preset)
    ...
    def _cleanup_expired_preset_rolls(self):
        """Remove preset test rolls older than 12 hours."""
        current_time = datetime.now()
        expiry_time = timedelta(hours=12)

        expired_count = 0
        users_to_remove = []
        for user_id, user_preset_rolls in self.preset_rolls.items():
            dice_exprs_to_remove = []

            for dice_expr, data in user_preset_rolls.items():
            ...
            for dice_expr in dice_exprs_to_remove:
                del user_preset_rolls[dice_expr]

            # Mark user for removal if no preset rolls left
            if not user_preset_rolls:
                users_to_remove.append(user_id)

        # Remove users with no preset rolls
        for user_id in users_to_remove:
            del self.preset_rolls[user_id]

        if expired_count > 0:
            log.debug(f"Cleaned up {expired_count} expired preset roll(s)")
    ...
    def _validate_preset_results(self, dice_expr: str, results: List[int]) -> tuple:
        """Validate that preset test results are possible for the given dice expression.
        ...
        if standard_match:
            ...
            # For preset results, we're setting the dice result before modifiers
    ...
    def _normalize_dice_key(self, dice_expr: str) -> str:
        ...
        return dice_part.lower()

    async def _handle_fudge_dice(self, ctx: commands.Context, roll_string: str, roll_type: str, preset_result: int = None):
        ...
        if preset_result is not None:
            ...
            dice_results = self._generate_fudge_dice_for_sum(num_dice, preset_result)
    ...
    async def _handle_fallout_dice(self, ctx: commands.Context, roll_string: str, roll_type: str, preset_result: int = None):
        ...
        if preset_result is not None:
            # For preset results on Fallout dice, just use the preset damage value
            # Generate random-looking dice that could sum to that damage
            total_damage = preset_result
            total_effects = 0
            dice_results = []
    ...
    async def _handle_preset_standard_dice(self, ctx: commands.Context, roll_string: str, preset_result: int, roll_type: str):
        """Handle preset test results for standard dice with advanced operations."""
        import re
        ...
        if not has_advanced:
            ...
            actual_total = preset_result + modifier
            modifier_str = ""
            if modifier > 0:
            ...
            display_result = f"{dice_part} ({preset_result}){modifier_str}"
