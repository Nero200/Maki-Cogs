...
class ChimeraDice(commands.Cog):
    """A dice roller with probability-based luck, karma, and GM control systems.
    ...
    - GM force rolling and campaign statistics
    """
    ...
    def __init__(self, bot):
        ...
        # In-memory storage for forced rolls with timestamps.
    ...
    async def _execute_roll(self, ctx: commands.Context, roll_string: str, roll_type: str):
        ...
        try:
            # Validate dice expression first
            is_valid, error_msg = self._validate_dice_expression(roll_string)
            if not is_valid:
                await ctx.send(f"Invalid dice expression: {error_msg}")
                return
                
            # Check for forced rolls first
            user_id = ctx.author.id
            forced_result = None
            
            if user_id in self.forced_rolls:
                # Clean up expired rolls first
                self._cleanup_expired_forced_rolls()
                
                # Check if we have a forced result for this exact expression or variations
                # Extract just the dice part (without modifiers and advanced operations) for lookup
                base_dice_part = self._extract_base_dice(roll_string)
                
                expressions_to_try = [
                    roll_string,                               # Full expression as typed
                    roll_string.lower(),                       # Lowercase full expression
                    base_dice_part,                            # Just base dice part
                    base_dice_part.lower(),                    # Lowercase base dice part
                    roll_string.replace(' ', '').lower(),      # Remove spaces
                    roll_string.replace('dF', 'df').lower(),   # Normalize fudge dice
                    roll_string.replace('dD', 'dd').lower(),   # Normalize fallout dice
                    base_dice_part.replace('dF', 'df').lower(), # Normalized base dice part
                    base_dice_part.replace('dD', 'dd').lower()  # Normalized base dice part
                ]
                
                for expr in expressions_to_try:
                    if expr in self.forced_rolls[user_id]:
                        data = self.forced_rolls[user_id][expr]
                        # Handle both new format (dict) and legacy format (list)
                        if isinstance(data, dict) and "values" in data and data["values"]:
                            forced_result = data["values"].pop(0)
                            # Remove entry if no more values
                            if not data["values"]:
                                del self.forced_rolls[user_id][expr]
                            break
                        elif isinstance(data, list) and data:
                            # Legacy format support
                            forced_result = data.pop(0)
                            if not data:
                                del self.forced_rolls[user_id][expr]
                            break
            
            # Handle special dice types first
            if 'df' in roll_string.lower() or 'f' in roll_string.lower():
                await self._handle_fudge_dice(ctx, roll_string, roll_type, forced_result)
                return
            elif 'dd' in roll_string.lower():
                await self._handle_fallout_dice(ctx, roll_string, roll_type, forced_result)
                return
            
            # Use d20 library for standard dice with full support for advanced operations
            if forced_result is not None:
                # For forced results with advanced operations, we need special handling
                result = await self._handle_forced_standard_dice(ctx, roll_string, forced_result, roll_type)
            else:
                # Apply bias for karma/luck rolls, otherwise roll normally
                if roll_type == "karma":
                    result = await self._roll_standard_dice_with_karma(ctx, roll_string)
                elif roll_type == "luck":
                    result = await self._roll_standard_dice_with_luck(ctx, roll_string)
                else:
            ...
            # Apply roll type modifications (but not if forced)
            ...
            await self._record_roll(ctx, roll_string, actual_total, roll_type, original_total)
            
            # Format output with visual indicators
            emoji = self._get_roll_emoji(roll_type)
            output = f"{emoji} **{ctx.author.display_name}** rolls `{roll_string}`...\n"
    ...
    @commands.dm_only()
    async def force(self, ctx: commands.Context, *, args: str):
        # Hidden GM command - Forces the result of next roll. DM-only for secrecy.
        # Usage: 
        # - >force 1d20 15 (force your own next roll)
        # - >force 123456789012345678 1d20 15 (force specific user by Discord ID)
        # Note: To get user IDs, enable Developer Mode in Discord settings.
        if not args:
            return await ctx.send("Usage:\n• `>force 1d20 15` (force your own roll)\n• `>force 123456789012345678 1d20 15` (force specific user by Discord ID)\nTip: Enable Developer Mode in Discord to copy user IDs")
        ...
        if target:
            ...
            if target.isdigit() or (target.startswith('<@') and target.endswith('>')):
                try:
                    ...
                    found_user = self.bot.get_user(target_user_id)
            ...
            # In DMs, only Discord IDs work (no guild context available)
                ...
                return await ctx.send(f"In DMs, you must use a Discord user ID. Example: `>force 123456789012345678 1d20 15`\nTo get a user's ID: Enable Developer Mode in Discord, right-click the user, and select 'Copy ID'.")
            
            if found_user:
                user_id = found_user.id
                target_name = f"{found_user.display_name}'s"
        
        # Store forced roll (normalize to just the dice part, without modifiers)
        dice_part, _ = self._parse_dice_modifiers(dice_expr)
        normalized_expr = dice_part.lower()
        
        if user_id not in self.forced_rolls:
            self.forced_rolls[user_id] = {}
        
        if normalized_expr not in self.forced_rolls[user_id]:
            self.forced_rolls[user_id][normalized_expr] = {
                "values": [],
                "timestamp": datetime.now()
            }
        
        # Add the new results and update timestamp
        self.forced_rolls[user_id][normalized_expr]["values"].extend(results)
        self.forced_rolls[user_id][normalized_expr]["timestamp"] = datetime.now()
        
        target_name = "your" if user_id == ctx.author.id else "their"
    ...
    async def _update_percentile_debt(self, ctx: commands.Context, original_result: int, modified_result: int):
        ...
        if percentile is not None:
            ...
            # If we applied a karma modification, reduce the debt by the correction amount
            if modified_result != original_result:
                modification_strength = abs(modified_result - original_result)
                # Estimate how many percentile points this modification represents
                estimated_percentile_correction = modification_strength * 8  # Rough estimate
                if new_debt > 0 and modified_result > original_result:
                    new_debt -= estimated_percentile_correction
                elif new_debt < 0 and modified_result < original_result:
                    new_debt += estimated_percentile_correction
            
            # Decay the debt slightly over time to prevent infinite accumulation
            decay_factor = 0.98
            new_debt *= decay_factor
    ...
    def _estimate_keep_percentile(self, roll_string: str, result: int, num_dice: int, die_size: int) -> float:
        ...
        if kh_match:
            ...
            # Drop highest X dice (equivalent to keep lowest Y where Y = num_dice - X)
            drop_count = int(dh_match.group(1))
            ...
            # Drop lowest X dice (equivalent to keep highest Y where Y = num_dice - X)
            drop_count = int(dl_match.group(1))
            keep_count = num_dice - drop_count
            min_result = keep_count
            max_result = keep_count * die_size
    ...
    def _cleanup_expired_forced_rolls(self):
        """Remove forced rolls older than 12 hours."""
        current_time = datetime.now()
        expiry_time = timedelta(hours=12)
        
        users_to_remove = []
        for user_id, user_forced_rolls in self.forced_rolls.items():
            dice_exprs_to_remove = []
            
            for dice_expr, data in user_forced_rolls.items():
            ...
            for dice_expr in dice_exprs_to_remove:
                del user_forced_rolls[dice_expr]
            
            # Mark user for removal if no forced rolls left
            if not user_forced_rolls:
                users_to_remove.append(user_id)
        
        # Remove users with no forced rolls
        for user_id in users_to_remove:
            del self.forced_rolls[user_id]
    ...
    async def _handle_fudge_dice(self, ctx: commands.Context, roll_string: str, roll_type: str, forced_result: int = None):
        ...
        if forced_result is not None:
            # Force the dice result directly (before adding bonus)
            dice_results = self._generate_fudge_dice_for_sum(num_dice, forced_result)
        ...
        emoji = self._get_roll_emoji(roll_type)
        output = f"{emoji} **{ctx.author.display_name}** rolls `{original_string}`...\n"
    ...
    async def _handle_fallout_dice(self, ctx: commands.Context, roll_string: str, roll_type: str, forced_result: int = None):
        ...
        if forced_result is not None:
            # For forced results on Fallout dice, just use the forced damage value
            # Generate random-looking dice that could sum to that damage
            total_damage = forced_result
            total_effects = 0
            dice_results = []
    ...
    async def stats(self, ctx: commands.Context, user: discord.Member = None):
        ...
        embed.add_field(
            name="Current Karma",
            value=user_data["current_karma"],
            inline=True
        )
        
        embed.add_field(
            name="Percentile Debt",
    ...
    async def admin_stats(self, ctx: commands.Context, user: discord.Member):
        ...
        embed.add_field(
            name="Settings",
            value=f"Set Luck: {user_data['set_luck']}\nCurrent Karma: {user_data['current_karma']}\nPercentile Debt: {user_data.get('percentile_debt', 0.0):+.1f}",
            inline=True
        )
    ...
    async def _roll_standard_dice_with_karma(self, ctx: commands.Context, roll_string: str):
        ...
        if simple_match and abs(debt) > 5.0:
            ...
            if num_dice <= 10:  # Reasonable limit
                ...
                class MockResult:
                    def __init__(self, total, dice_results, modifier):
                        self.total = total
                ...
                return MockResult(final_total, dice_results, modifier_value)
    ...
    async def _roll_standard_dice_with_luck(self, ctx: commands.Context, roll_string: str):
        ...
        if simple_match and abs(luck_debt) > 2.0:  # Lower threshold for luck
            num_dice = int(simple_match.group(1))
            die_size = int(simple_match.group(2))
            modifier_sign = simple_match.group(3)
            modifier_value = int(simple_match.group(4)) if simple_match.group(4) else 0
    ...
    def _translate_dice_syntax(self, expression: str) -> str:
        ...
        # Match patterns like: 2d20dl1, 4d6dl1+5, 3d8dl2-1
        ...
        # Match patterns like: 3d20dh1, 5d8dh2+3
    ...
    async def _handle_forced_standard_dice(self, ctx: commands.Context, roll_string: str, forced_result: int, roll_type: str):
        """Handle forced results for standard dice with advanced operations."""
        import re
        ...
        if not has_advanced:
            # Simple case - just force the basic dice result
            dice_part, modifier = self._parse_dice_modifiers(roll_string)
            
            # Create a mock result object similar to d20.roll result
            class MockResult:
                def __init__(self, total, result_str):
                    self.total = total
                    self.result = result_str
            
            actual_total = forced_result + modifier
            modifier_str = ""
            if modifier > 0:
            ...
            display_result = f"{dice_part} ({forced_result}){modifier_str}"
            return MockResult(actual_total, display_result)
        else:
            # Complex case - we can't easily force the result with advanced operations
